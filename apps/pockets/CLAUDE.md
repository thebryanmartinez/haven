...
Pockets subdivides one bank account into multiple "funds" (Savings, Groceries, Rent)
...
```bash
pnpm dev --filter pockets     # dev server on port 3000
pnpm build --filter pockets   # production build
```
...
use `getNewBalance` from `@haven/backend/lib/balance`.
